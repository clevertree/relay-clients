...
function JsonBlock({ value }: { value: any }) {
  ...
  return <TSDiv tag="pre" className="font-mono text-xs bg-black/5 p-2 rounded overflow-auto max-h-48 whitespace-pre-wrap">{text}</TSDiv>
}
...
export default function StyleDebugPanel() {
  ...
  return (
    <TSDiv className="p-3 mt-4 border rounded bg-[var(--bg-surface)] text-sm">
      {wasmError ? (
        <TSDiv className="mb-3 p-3 bg-[var(--bg-error)] border border-[var(--border-error)] rounded">
          <TSDiv className="font-semibold text-[var(--text-error)]">Themed-styler WebAssembly failed to load</TSDiv>
          <TSDiv className="text-xs text-[var(--text-error)] mt-1">Diagnostics:</TSDiv>
          <TSDiv className="mt-2 text-xs">
            <TSDiv><TSDiv tag="strong">Message:</TSDiv> {wasmError.message}</TSDiv>
            {wasmError.stack ? <TSDiv tag="pre" className="font-mono text-xs bg-black/5 p-2 rounded mt-2 overflow-auto max-h-40 whitespace-pre-wrap">{wasmError.stack}</TSDiv> : null}
            <TSDiv className="mt-2 text-xs text-gray-600">Captured at: {wasmError.time}</TSDiv>
            <TSDiv className="mt-2 text-xs text-[var(--text-secondary)]">Suggested actions: ensure the wasm build step ran and that the wasm file is served correctly; check browser console/network for errors.</TSDiv>
          </TSDiv>
        </TSDiv>
      ) : null}
      <TSDiv className="flex items-center justify-between mb-2">
        ...
        <TSDiv className="space-x-2">
          <TSDiv tag="button" onClick={refresh} className="px-2 py-1 bg-blue-500 text-white rounded text-xs">Refresh</TSDiv>
          <TSDiv tag="button" onClick={forceLoadFromManifest} className="px-2 py-1 bg-green-500 text-white rounded text-xs">Force manifest load</TSDiv>
          <TSDiv tag="button" onClick={copyCss} className="px-2 py-1 bg-[var(--bg-secondary)] rounded text-xs">Copy CSS</TSDiv>
        </TSDiv>
      </TSDiv>

      <TSDiv className="grid grid-cols-2 gap-3">
        <TSDiv>
          <TSDiv className="font-semibold mt-3 mb-1">Themed-styler version</TSDiv>
          <JsonBlock value={themedStylerVersion} />
          <TSDiv className="font-semibold mb-1">Generated CSS</TSDiv>
          <TSDiv tag="pre" className="font-mono text-xs bg-black/5 p-2 rounded overflow-auto max-h-72 whitespace-pre-wrap">{css}</TSDiv>
          <TSDiv className="text-xs text-gray-600 mt-2">(Diagnostics)</TSDiv>
          <TSDiv className="text-xs mt-1">Global version: <TSDiv tag="span" className="font-mono">{String(themedStylerGlobalVersion ?? 'null')}</TSDiv></TSDiv>
          <TSDiv className="text-xs mt-1">Module-reported: <TSDiv tag="span" className="font-mono">{String(themedStylerModuleVersion ?? 'null')}</TSDiv></TSDiv>
          {themedStylerWasmInfo ? (
            <TSDiv className="mt-2 text-xs">
              <TSDiv>WASM URL: <TSDiv tag="span" className="font-mono break-all">{themedStylerWasmInfo.url}</TSDiv></TSDiv>
              <TSDiv>HTTP status: {themedStylerWasmInfo.status ?? 'n/a'}</TSDiv>
              <TSDiv>Content-Length: {themedStylerWasmInfo?.headers?.['content-length'] ?? 'n/a'}</TSDiv>
              <TSDiv>Last-Modified: {themedStylerWasmInfo?.headers?.['last-modified'] ?? 'n/a'}</TSDiv>
              <TSDiv>ETag: {themedStylerWasmInfo?.headers?.etag ?? 'n/a'}</TSDiv>
            </TSDiv>
          ) : null}
