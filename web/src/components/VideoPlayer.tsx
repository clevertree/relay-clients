...
import {DistributedFileLoader, type FileLoadResult, type LoadingStatus} from './DistributedFileLoader'
...
function StatusDisplay({status, message, progress}: { status: LoadingStatus; message: string; progress?: number }) {
    ...
    return (
        <TSDiv className="mb-3 space-y-2">
            ...
            {progress !== undefined && progress < 100 && (
                <TSDiv className="w-full h-1 rounded-full overflow-hidden">
...
function ErrorDisplay({error, message}: { error: string; message: string }) {
    ...
    return (
        <TSDiv className="mb-3 space-y-2">
            ...
            {expanded && message && (
                <TSDiv tag="pre"
                       className="text-xs text-red-300 bg-red-900 bg-opacity-20 p-2 rounded max-h-48 overflow-auto whitespace-pre-wrap break-words font-mono">
                    {message}
                </TSDiv>
            )}
        </TSDiv>
...
export function VideoPlayer({
    ...
    if (!isEnabled) {
        ...
        return (
            <TSDiv className={`w-full max-w-4xl rounded-lg border border-dashed bg-black/60 p-6 ${className || ''}`}
                   style={style}>
                <TSDiv className="text-center space-y-2">
                    <TSDiv className="text-lg font-semibold">Video playback temporarily disabled</TSDiv>
                    <TSDiv className="text-sm">The VideoPlayer UI is currently disabled while we stabilize streaming.
                        The underlying loader and playback code remain present and will be used when re-enabled.</TSDiv>
                    <TSDiv className="text-xs">Tip: set <TSDiv tag="code">window.__videoEnabled = true</TSDiv> in the
                        console to re-enable for testing.</TSDiv>
                </TSDiv>
            </TSDiv>
        )
    }
    ...
    return (
        <TSDiv className={`w-full max-w-4xl space-y-3 ${className || ''}`} style={style}>
            ...
            {isReady && resolvedSrc ? (
                ...
                <TSDiv className="w-full rounded aspect-video flex items-center justify-center">
